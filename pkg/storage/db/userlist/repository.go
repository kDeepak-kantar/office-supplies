...
func (r *service) SendRemainder() (map[string]interface{}, error) {
	...
	for _, order := range orders {
		...
		if today.AddDate(0, 0, -3).Before(dueDate) {
			...
			for _, userItr := range adminUsers {
				reminder := fmt.Sprintf("Dear %s,This is a reminder that there is a task with ID %d that is due in less than 3 days on %s. Please take necessary actions to approve it.Best regards,Admin", userItr.Name, order.ID, order.DueDate)
